...
class DDAD:
    def __init__(self, unet, config, category) -> None:
    ...
    def __call__(self, category) -> Any:
        feature_extractor = domain_adaptation(self.unet, self.config, fine_tune=False, category=category)
        ...
        if self.config.metrics.auroc:
            print('AUROC: (Image AUROC:{:.1f}, Pixel AUROC:{:.1f})'.format(metric.image_auroc() * 100, metric.pixel_auroc() * 100))
        ...
        if self.config.metrics.visualisation:
            visualize(forward_list, reconstructed_list, gt_list, pred_mask, anomaly_map_list, category, self.config.data.name)
