...
def train(config, category):
    ...
    trainer(unet, category, config)#config.data.category,
...
def detection(config, category):
    ...
    checkpoint = torch.load(os.path.join(os.getcwd(), config.model.checkpoint_dir, category, str(config.model.load_chp)))
    unet.eval()
    ddad = DDAD(unet, config, category)
    ddad(category)
...
def finetuning(config, category):
    ...
    unet.to(config.model.device)
    unet.eval()
    domain_adaptation(unet, config, fine_tune=True, category=category)
...
def parse_args():
    ...
    cmdline_parser.add_argument('-cfg', '--config', 
                                default= os.path.join(os.path.dirname(os.path.abspath(__file__)),'mvtec_config.yaml'),
                                help='config file')
...
if __name__ == "__main__":
    ...
    for cat in config.data.category:
        ...
        if args.domain_adaptation:
            ...
            finetuning(config, cat)
        if args.detection:
            print('Detecting Anomalies...')
            detection(config, cat)
