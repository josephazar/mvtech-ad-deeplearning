...
mvtec_root_path = data.get('mvtec_root_path')

if mvtec_root_path:
    dataset_name = os.path.basename(mvtec_root_path)
    print(f"Dataset name: {dataset_name}")
else:
    print("mvtec_root_path not found in the JSON file.")

with open('./args/distribution.yml', 'r') as file:
    data = yaml.load(file, Loader=yaml.FullLoader)

def get_texture_list(name):
    for entry in data:
        if entry.get('name').lower().strip() == name.lower().strip():
            return entry.get('texture_list', [])
    return []  # Return an empty list if the name is not found


texture_list = get_texture_list(dataset_name)

print('textureList',texture_list)
...
class MVTecTrainDataset(Dataset):
    ...
    def __init__(self, data_path,classname,img_size,args):
        ...
        foreground_path="/home/Dev-YoussefH/Desktop/UFC-Research/datasets/Modified_Format/thresh/"

        self.textural_foreground_path = sorted(glob.glob(foreground_path +"*.png"))
