...
def main():
    ...
    if os.path.exists(args["mvtec_root_path"]):
        mvtec_classes = [folder for folder in os.listdir(args["mvtec_root_path"]) if
                     os.path.isdir(os.path.join(args["mvtec_root_path"], folder))]
    else:
        print(f"Error: The path '{args['mvtec_root_path']}' does not exist.")
        mvtec_classes = []

    #mvtec_classes = ['grey_cloth']
